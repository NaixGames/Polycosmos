...
- Now you can choose between running from the compilled version or from source. The first is easier to use, while the second one guarantees better compatibility. For the compilled version, unzip the HadesCompilled.zip on your installed archipelago folder (where you see a bunch of Clients.exe type of files). For the source version, on your [source code Archipelago folder](https://github.com/ArchipelagoMW/Archipelago), copy HadesClient.py on the base folder (where all the Clients.py are) and copy the hades folder in the worlds folder.
